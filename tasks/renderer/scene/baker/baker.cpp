...
void Baker::bakeScene(std::filesystem::path path, std::filesystem::path output)
{
...

  {
    ...
    for (size_t i = 0; i < model.meshes.size(); ++i)
    {
      ...
      for (size_t j = 0; j < mesh.primitives.size(); ++j)
      {
        ...
        vertex_attrs[0].minValues.assign(relem.box.posMin.begin(), relem.box.posMin.end());
        vertex_attrs[0].maxValues.assign(relem.box.posMax.begin(), relem.box.posMax.end());
        ...
        for (size_t k = 0; k < 4; ++k)
        {
          ...
          {
            spdlog::warn(
              "Primitive {} for mesh {} doesn't have the {} attribute", j, i, attr_names[k]);
            continue;
          }
...
static void updateMinMax(RenderElement& relem, glm::vec3 curr)
{
  ...
  {
    relem.box.posMin[i] = std::min(relem.box.posMin[i], curr[i]);
    relem.box.posMax[i] = std::max(relem.box.posMax[i], curr[i]);
  }
}
  ...
  for (const auto& mesh : model.meshes)
  {
    ...
    for (const auto& prim : mesh.primitives)
    {
      ...
      result.relems.push_back(RenderElement{
        ...
        .box = {
          .posMax =
            {std::numeric_limits<float>::lowest(), // fuck numeric limits
